...
class AudioSynthesizerViewModel : ViewModel() {
    ...
    fun setFrequencySliderPosition(frequencySliderPosition: Float) {
        ...
        viewModelScope.launch {
            audioSynthesizer?.setFrequency(frequencyInHz)
        }
    }
    ...
    fun sliderPositionFromFrequencyInHz(frequencyHz: Float): Float {
        val rangePosition = rangePositionFromValue(range = frequencyRange, value = frequencyHz)
    ...
    fun setVolume(volumeInDb: Float) {
        ...
        viewModelScope.launch {
            audioSynthesizer?.setVolume(volumeInDb)
        }
    }

    private var _wavetable = Wavetable.SINE
    fun setWaveTable(newWavetable: Wavetable) {
        _wavetable = newWavetable
        viewModelScope.launch {
            audioSynthesizer?.setWaveTable(newWavetable)
        }
    }
    ...
    fun playClicked() {
        viewModelScope.launch {
            if (audioSynthesizer?.isPlaying() == true) {
                audioSynthesizer?.stop()
            } else {
                audioSynthesizer?.play()
            }
    ...
    fun applyParameters() {
        viewModelScope.launch {
            audioSynthesizer?.let {
                it.setFrequency(frequency.value!!)
                it.setVolume(volume.value!!)
                it.setWaveTable(_wavetable)
            }
            updatePlayButtonLabel()
        }
    }
